...
final class Repository implements RepositoryInterface
{
    ...
    public function __construct(
        ConnectionPoolInterface|PDO $connection,
    ) {
        $this->pool = $connection instanceof PDO
            ? new PdoConnectionPool($connection)
            : $connection;
    }
    ...
    public function findByQuery(DbQueryBuilderInterface $query): array
    {
        ...
        if (is_string($prepared)) {
            throw new \InvalidArgumentException('findByQuery requires a prepared query (prepared: true)');
        }
